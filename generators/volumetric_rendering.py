...
def fancy_integration(rgb_sigma, z_vals, device, noise_std=0.5, last_back=False, white_back=False, black_back=False, clamp_mode=None, fill_mode=None, fill_color='black'):
    ...
    if black_back:
        rgb_final = rgb_final + (1-weights_sum) * -1 # black means 0
        # rgb_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([0.]*22, device=rgb_final.device)

    if fill_mode == 'debug':
        rgb_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1.]+[0.]*21, device=rgb_final.device)
        weights_sum = weights_sum.expand_as(rgb_final)
        ...
        return rgb_final, depth_final, weights_sum

    elif fill_mode == 'weight_debug':
        rgb_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1.]+[0.]*21, device=rgb_final.device)
        weights_sum = weights_sum.expand_as(rgb_final)
        # return rgb_final, depth_final, weights, weights_sum
        # debug:
    ...
    elif fill_mode == 'seg_padding_background':
        background_padding = torch.zeros((rgb_final.shape[0], rgb_final.shape[1], 1), device=rgb_final.device)
        rgb_final = torch.cat([background_padding, rgb_final], dim=-1)
        if fill_color == 'white':
            rgb_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1.]+[1.]*21, device=rgb_final.device)
        elif fill_color == 'black':
            rgb_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1.]+[0.]*21, device=rgb_final.device)
        elif fill_color == 'grey':
            rgb_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1.]+[0.5]*21, device=rgb_final.device)
        elif fill_color == 'light_grey':
            rgb_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1.]+[0.81]*21, device=rgb_final.device)
        weights_sum = weights_sum.expand_as(rgb_final)
        return rgb_final, depth_final, weights

    elif fill_mode == 'eval_seg_padding_background':
        background_padding = torch.zeros((rgb_final.shape[0], rgb_final.shape[1], 1), device=rgb_final.device)
        rgb_final = torch.cat([background_padding, rgb_final], dim=-1)
        if fill_color == 'white':
            rgb_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1.]+[1.]*21, device=rgb_final.device)
    ...
    elif fill_mode == 'eval_white_back':
        rgb_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1., 1., 1.], device=rgb_final.device)
        weights_sum = weights_sum.expand_as(rgb_final)
        return rgb_final, depth_final, weights_sum
